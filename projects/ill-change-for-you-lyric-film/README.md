...
The supplied text is retained in [source/lyrics-supplied.txt](source/lyrics-supplied.txt). The [timeline](src/timeline.json) currently contains 25 display lines and 118 individually identified word intervals at 44.1 kHz. It preserves isolated opening words, two complete refrain performances and the closing phrase. The [preliminary alignment record](evidence/alignment-candidate-preliminary.json) keeps its model-assisted status and candidate basis. Its boundaries are provisional until checked against the actual mix; passing interval validation is not proof of acoustic accuracy.
