...
def main() -> None:
    ...
    assert SOURCE.stat().st_size == manifest["byteLength"], "Source byte count differs from locked manifest"
    video_probe = json.loads(subprocess.check_output([
        "ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries",
        "stream=codec_name,width,height,sample_aspect_ratio,display_aspect_ratio,r_frame_rate,avg_frame_rate,time_base,start_pts,duration_ts,nb_frames",
        "-of", "json", str(SOURCE),
    ]))["streams"][0]
    video = manifest["video"]
    assert (video_probe["codec_name"], video_probe["width"], video_probe["height"]) == (
        "h264", video["width"], video["height"]), "Video stream differs from locked manifest"
    for probe_key, manifest_key in (
        ("sample_aspect_ratio", "sampleAspectRatio"),
        ("display_aspect_ratio", "displayAspectRatio"),
        ("r_frame_rate", "nominalFrameRate"),
        ("avg_frame_rate", "averageFrameRate"),
        ("time_base", "timeBase"),
    ):
        assert video_probe[probe_key] == video[manifest_key], f"Video {probe_key} differs from locked manifest"
    assert int(video_probe["start_pts"]) == video["startPts"]
    assert int(video_probe["duration_ts"]) == video["durationTs"]
    assert int(video_probe["nb_frames"]) == video["frameCount"]
    ...
    assert sample_rate == manifest["audio"]["sampleRate"]
    assert channels == manifest["audio"]["channels"]
    assert int(probe["duration_ts"]) == manifest["audio"]["durationTs"]
    assert probe["time_base"] == manifest["audio"]["timeBase"]
    ...
    assert sample_count == manifest["audio"]["sampleCount"]
    ...
    # state. Include boundaries and widely spaced checkpoints in this recording.
    rng = np.random.default_rng(20260924)
    seeks = np.unique(np.r_[0, 1, frame_count - 2, frame_count - 1,
                           np.rint(np.array([10, 30, 60, 90, 120, 150, 180]) * FPS).astype(int),
    ...
    metadata = {
        ...
        "analysis": {
            ...
            "frequencyRangeReason": "A 20 Hz–16 kHz range covers the useful spectrum of this locked AAC recording. Per-band occupancy and percentiles are recorded below; low and high bands are not asserted to be independent vocal measurements.",
            "fftBinHz": hz_per_bin, "integration": "fractional overlap of each log band with FFT-bin frequency cells",
            "resolutionCaveat": f"Low bands narrower than {hz_per_bin:.6f} Hz share FFT bins; all bands have nonzero coverage, but low bands are correlated, not independent narrow filters.",
    ...
    audit = {
        ...
        "limits": [
            ...
            f"A time-centered {FFT_SIZE / sample_rate * 1000:.2f} ms window has {FFT_SIZE / sample_rate * 500:.2f} ms of support on either side; it introduces no causal playback delay, but cannot distinguish events below that analysis resolution.",
