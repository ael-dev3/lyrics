# Posting assets

The local kit has separate YouTube and TikTok directories, each with its complete film, title, description and dedicated cover. Optional Spanish, English and bilingual SRT files accompany the burned-in word-focused presentation.

- YouTube thumbnail: 1920×1080 JPEG.
- TikTok profile cover: **1200×1600 portrait**, 3:4 width:height.
- Both covers use a frame from the original live performance at 00:24.000 and the approved warm serif palette. No generated replacement imagery is used.
- YouTube 320px, TikTok 150×200px and a conservative 5% crop simulation were checked for complete title, correct Fémina accent and recognizable performer. These are local checks, not a platform-upload preview.

Music and performance: Fémina. Production: La Oreja Negra. Film: FLAN Audiovisual. Source-image rights are excluded from the repository contribution license. Platform descriptions credit the source and disclose AI-assisted lyric preparation. Full media stays local; no platform upload or public media release is included in source integration.

[Cover identities](../evidence/cover-assets.json) · [YouTube title](YouTube-Title.txt) · [YouTube description](YouTube-Description.txt) · [TikTok description](TikTok-Description.txt)
