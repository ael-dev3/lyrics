...
for(const format of ['landscape','portrait'] as const)for(const at of moments){
 ...
 await renderStill({serveUrl,composition:{...composition,props:{format,at}},inputProps:{format,at},output:`evidence/stills/v2-${format}-${Math.round(at*60)}.png`,imageFormat:'png',logLevel:'warn'});
