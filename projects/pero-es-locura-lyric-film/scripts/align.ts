...
if mode=='whisper':
 ...
 model=stable_whisper.load_model(modelname,device='cpu')
else:
...
for s in windows:
 if mode=='whisper':
  result=model.align_words(samples,[{k:s[k] for k in ['start','end','text']}],language='es',regroup=False,suppress_silence=False,verbose=None)
  ...
  tokens=[re.sub("[^a-z']",'',roman.romanize_string(w.lower(),lcode='spa')) for w in sourcewords]
 ...
 output.append(item)
 json.dump({'method':mode,'model':modelname if mode=='whisper' else 'MMS_FA','audio':audio,'windowFile':windowfile,'segments':output},open('analysis/'+mode+'-'+audio+tag+'.json','w'),ensure_ascii=False,indent=2)
