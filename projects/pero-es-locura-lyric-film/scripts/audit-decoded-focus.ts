...
GlobalFonts.registerFromPath('public/fonts/CormorantGaramond-Semibold.ttf','LiveSerif');
const all=Object.values(l.cues).flatMap(c=>[...c.es,...c.en]);
...
for(const cue of data.cues){const layout=l.cues[cue.id]!;for(const w of [...layout.es,...layout.en]){
 const size=l.fontSize,canvas=createCanvas(Math.ceil(w.width+8),Math.ceil(size*1.5)),ctx=canvas.getContext('2d');ctx.font=`600 ${size}px LiveSerif`;ctx.fillStyle='white';ctx.fillText(w.text,w.x-Math.floor(w.x)+3,size);const rgba=ctx.getImageData(0,0,canvas.width,canvas.height).data;
