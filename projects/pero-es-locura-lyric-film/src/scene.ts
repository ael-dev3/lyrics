...
export function lyricSvg(frame:number,format:Format,data:ProductionData,layouts:Layouts){
 ...
 return '<g data-cue="'+c.id+'" filter="url(#type-shadow)">'+words(p.es,activeSource(c,frame,data),'es')+words(p.en,activeTargets(c,frame,data),'en')+'</g>';
}
export function sceneSvg(frame:number,format:Format,data:ProductionData,layouts:Layouts,bands:readonly number[][]){
 ...
 return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 '+w+' '+h+'" width="'+w+'" height="'+h+'" style="font-family:LiveSerif;font-weight:600;font-synthesis:none">'+defs+(p?'':'<rect data-decor="shade" width="'+w+'" height="'+h+'" fill="url(#shade)" opacity="'+alpha+'"/>')+metadata+'<g id="lyric-layer">'+lyricSvg(frame,format,data,layouts)+'</g>'+bars+'</svg>';
}
