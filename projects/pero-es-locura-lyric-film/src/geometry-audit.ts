...
import {visibleCue} from './focus.ts';
...
await document.fonts.load('600 82px LiveSerif');await document.fonts.load('600 24px LiveSerif');
...
for(const format of ['landscape','portrait'] as const){
 ...
 for(const cue of data.cues){
  const times=[...new Set([cue.startSample,...cue.es.flatMap(w=>[Math.round((w.startSample+w.endSample)/2),...[w.startSample,w.endSample].flatMap(s=>[-1,0,1].map(offset=>Math.round((Math.round(s/data.sampleRate*60)+offset)/60*data.sampleRate)))])])].filter(s=>s>=cue.visibleFrom&&s<cue.visibleUntil);let baseline:string|undefined;
  for(const time of times){
   const frame=Math.round(time/data.sampleRate*60);if(visibleCue(data,frame)?.id!==cue.id)continue;painter.paint(frame,format);states++;const reference=sceneSvg(frame,format,data,layouts,bands);const expected=new Map([...reference.matchAll(/data-word="([^"]+)"[^>]*fill="([^"]+)"/g)].map(m=>[m[1],m[2]]));for(const word of host.querySelectorAll('[data-word]'))if(word.getAttribute('fill')!==expected.get(word.getAttribute('data-word')??''))failures.push(`${format}/${cue.id}: painter focus mismatch`);const referenceDoc=new DOMParser().parseFromString(reference,'image/svg+xml');for(const id of ['metadata','shade','spectrum'])if(host.querySelector('[data-decor="'+id+'"]')?.getAttribute('opacity')!==referenceDoc.querySelector('[data-decor="'+id+'"]')?.getAttribute('opacity'))failures.push('Decorator mismatch '+id);
   const expectedBars=[...referenceDoc.querySelectorAll('[data-band]')];for(const bar of host.querySelectorAll('[data-band]'))for(const attr of ['height','y'])if(bar.getAttribute(attr)!==expectedBars[Number(bar.getAttribute('data-band'))]?.getAttribute(attr))failures.push('Painter spectrum mismatch');
   const words=[...host.querySelectorAll<SVGTextElement>('text[data-word]')];
   if(words.length!==cue.es.length+cue.en.length)failures.push(`${format}/${cue.id}: incomplete visible text`);
   const boxes=words.map(w=>{const r=w.getBBox();ctx.font=`600 ${l.fontSize}px LiveSerif`;const ink=ctx.measureText(w.textContent??'');const baseline=Number(w.getAttribute('y'));return {id:w.dataset.word!,x:r.x,y:baseline-ink.actualBoundingBoxAscent,width:r.width,height:ink.actualBoundingBoxAscent+ink.actualBoundingBoxDescent,baseline};});glyphs+=boxes.length;
