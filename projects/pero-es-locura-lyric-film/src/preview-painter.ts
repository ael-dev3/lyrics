...
import {sceneSvg,lyricSvg,spectrum,decorOpacity} from './scene.ts';
...
export function createPreviewPainter(host:HTMLElement,data:ProductionData,layouts:Layouts,bands:readonly number[][]){
 ...
 return {paint(frame:number,format:Format){
  ...
  if(cue){const ids=new Set([...activeSource(cue,frame,data),...activeTargets(cue,frame,data)]);for(const word of wordNodes){const on=ids.has(word.dataset.word??'');if(word.dataset.active!==String(on)){word.dataset.active=String(on);word.setAttribute('fill',on?palette.active:palette.rest);}}}
  const alpha=decorOpacity(frame/data.fps);for(const el of decor)el.setAttribute('opacity',String(alpha*(el.dataset.decor==='spectrum'?.68:1)));
