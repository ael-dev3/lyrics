...
The project owner commissioned these visual corrections and a new local posting delivery after v2 was completed. This is recorded as `owner-directed-visual-revision`, with `previewAccepted: false` for v3; it does not invent a new overall acceptance or granular listening record. The live corrected preview was technically checked before rendering. Original timing uncertainty and incomplete listening fields remain unchanged. The gate requires the prior audio, text, mappings and geometry to match, plus current source identities and visual evidence. This narrow correction record does not change the preview-first default for new songs.
