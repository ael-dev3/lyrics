import {stars,starOpacity} from '../src/stars.ts';
...
export async function compositor(format:Format){
 ...
 return {canvas:out,async paint(frame:number){ctx.clearRect(0,0,canvas.width,canvas.height);const m=lunarState(frame,data),rounded=(n:number)=>Number(n.toFixed(4));
  ctx.save();ctx.scale(2,2);for(const s of stars(format)){ctx.globalAlpha=starOpacity(s.hr,s.opacity,frame);const radius=Number((s.radius*5).toFixed(3)),gradient=ctx.createRadialGradient(s.x,s.y,0,s.x,s.y,radius);gradient.addColorStop(0,'rgba(245,245,245,0.36)');gradient.addColorStop(.25,'rgba(245,245,245,0.12)');gradient.addColorStop(1,'rgba(245,245,245,0)');ctx.fillStyle=gradient;ctx.beginPath();ctx.arc(s.x,s.y,radius,0,Math.PI*2);ctx.fill();ctx.fillStyle='#F5F5F5';ctx.beginPath();ctx.arc(s.x,s.y,s.radius,0,Math.PI*2);ctx.fill();}ctx.restore();await layer('moon-atmosphere',rounded(m.haloOpacity));await layer('moon-light',rounded(m.moonOpacity));await layer('moon-shade',rounded((1-m.reveal)*.88));
