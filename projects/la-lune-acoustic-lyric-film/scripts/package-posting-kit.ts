...
const root='output/Posting Kit v3';mkdirSync(root,{recursive:true});
...
for(const [format,platform,dimensions] of [['landscape','YouTube','1920x1080'],['portrait','TikTok','1080x1920']] as const){
 const name=`La-Lune-Celestial-v3-${format}-${dimensions}-60fps.mp4`,source='output/'+name,technical=read(`evidence/${name}.verification.json`),focus=read(`evidence/${format}-${name}.focus-verification.json`);
...
const receipt={status:'verified local posting kit',song:identity.song,revision:identity.revision,sourceCommit:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),previewHashes:identity.hashes,videos,files:inventory,scope:'Technical verification and source-event display agreement are complete. Owner-directed visual revision; prior timeline acceptance is separate from incomplete granular listening telemetry. No public media release or platform upload.',builtAt:new Date().toISOString()};
