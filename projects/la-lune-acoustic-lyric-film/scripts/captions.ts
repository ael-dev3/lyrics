...
for(const language of ['fr','en','bilingual'] as const){const text=data.cues.map((c,i)=>{const ru=c.fr.map(w=>w.text).join(' '),en=c.en.map(w=>w.text).join(' ');return `${i+1}\n${stamp(c.startSample)} --> ${stamp(c.endSample)}\n${language==='fr'?ru:language==='en'?en:ru+'\n'+en}\n`;}).join('\n');writeFileSync(`publishing/captions/La-Lune-${language}.srt`,text);}
