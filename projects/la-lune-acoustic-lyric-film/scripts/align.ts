...
for s in windows:
 if mode=='whisper':
  result=model.align_words(samples,[{k:s[k] for k in ['start','end','text']}],language='fr',regroup=False,suppress_silence=False,verbose=None)
  ...
  tokens=[re.sub("[^a-z']",'',roman.romanize_string(w.lower(),lcode='fra')) for w in sourcewords]
