...
export const productionPaths=[...identityPaths,'src/Film.tsx','src/production-gate.ts','scripts/production-contract.ts','scripts/production-gate.ts','scripts/render.ts','evidence/final-sync-audit.json','evidence/production-adapter-equivalence.json','src/raster-inventory.ts','src/RasterCache.tsx','scripts/raster-cache.ts','scripts/raster-compose.ts','scripts/raster-contract.ts','scripts/render-raster.ts','scripts/raster-proof.ts','scripts/adopt-raster.ts','evidence/raster-adoption.json'] as const;
...
export function assertProductionGate(){
 ...
 const audit=object(read('evidence/final-sync-audit.json')),adapter=object(read('evidence/production-adapter-equivalence.json'));if(audit.status!=='passed technical and editorial audit'||adapter.status!=='PASS')throw Error('Required pre-render audit missing');
