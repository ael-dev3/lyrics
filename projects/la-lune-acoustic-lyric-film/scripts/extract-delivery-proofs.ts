...
const width=format==='landscape'?1920:1080,height=format==='landscape'?1080:1920,file=`output/La-Lune-Celestial-v3-${format}-${width}x${height}-60fps.mp4`;
...
writeFileSync(`evidence/${format}-decoded-stills.json`,JSON.stringify({revision:'preview-v3-celestial',source:file,frames:inventory,scope:'Frames decoded from the final delivery; inspected checkpoints do not certify every raster frame or acoustic boundary.'},null,2)+'\n');
