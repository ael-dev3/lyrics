...
import {sceneSvg,spectrum} from '../src/scene.ts';
...
assert.deepEqual(tokens((read('source/lyrics-fr.json') as string[]).join(' ')),tokens(d.cues.flatMap(c=>c.fr.map(w=>w.text)).join(' ')),'Supplied lyric coverage');
assert.equal(d.frames,Math.ceil(d.sampleCount/d.sampleRate*d.fps));assert.equal(bands.length,d.frames);const manifest=read('analysis/manifest.json');assert.equal(manifest.sampleRate,d.sampleRate);assert.equal(manifest.sampleCount,d.sampleCount);assert.equal(manifest.sourceAudioSha256,d.audioSha256);
...
for(let ci=0;ci<d.cues.length;ci++){
 ...
 for(let i=0;i<c.fr.length;i++){const w=c.fr[i]!,before=c.fr[i-1];if(before)assert.ok(before.endSample<=w.startSample,`source overlap ${w.id}`);assert.ok(w.reviewRequired,'Unreviewed model candidates must stay flagged');for(const s of [w.startSample,w.endSample])maxQuantizationMs=Math.max(maxQuantizationMs,Math.abs(Math.round(s/d.sampleRate*d.fps)/d.fps-s/d.sampleRate)*1000);}
 const frames=new Set(c.fr.flatMap(w=>[w.startSample,w.endSample].flatMap(s=>{const f=Math.round(s/d.sampleRate*d.fps);return [f-1,f,f+1];})));
 for(const format of ['landscape','portrait'] as const){
  const l=layouts[format],geometry=l.cues[c.id]!;assert.deepEqual(geometry.fr.map(w=>w.id),c.fr.map(w=>w.id));assert.deepEqual(geometry.en.map(w=>w.id),c.en.map(w=>w.id));
  for(const f of frames){
   ...
   for(const word of [...c.fr,...c.en]){
    if(!visible){assert.ok(!colors.has(word.id));continue;}
    const source='sourceIds'in word?c.fr.filter(w=>word.sourceIds.includes(w.id)):[word];
    const on=source.some(w=>f>=Math.round(w.startSample/d.sampleRate*d.fps)&&f<Math.round(w.endSample/d.sampleRate*d.fps));
    assert.equal(colors.get(word.id),on?palette.accent:palette.ivory,`${word.id}/${format}/${f}`);checkedWords++;
   }testedStates++;
  }
 }
 semantic.push({id:c.id,fr:c.fr,en:c.en.map(w=>({...w,sourceIntervals:c.fr.filter(s=>w.sourceIds.includes(s.id)).map(s=>({startSample:s.startSample,endSample:s.endSample})),highlightRule:'union, exclusive ends, no invented English times'}))});
}
for(let f=0;f<d.frames;f++){for(const portrait of [false,true])for(const v of spectrum(f,bands,portrait))assert.ok(Number.isFinite(v)&&v>=1.5&&v<=35.5);}
assert.ok(maxQuantizationMs<=1000/d.fps/2+.0001);
const result={status:'PASS',cues:d.cues.length,sourceTokens:d.cues.reduce((n,c)=>n+c.fr.length,0),targetTokensIncludingPunctuation:d.cues.reduce((n,c)=>n+c.en.length,0),testedBoundaryStatesBothFormats:testedStates,checkedVisibleWordColors:checkedWords,maxFrameQuantizationMs:maxQuantizationMs,frames:d.frames,clockSampleRate:d.sampleRate,clockSamples:d.sampleCount,meaningCoverage:'Every supplied source token and every target token mapped; semantic exceptions regression-tested',scope:'Structural, signal, timing-event and SVG color verification. Not a completed listening review or encoded-video verification.'};
