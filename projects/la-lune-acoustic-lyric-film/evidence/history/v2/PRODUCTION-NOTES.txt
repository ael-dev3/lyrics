...
The [segment continuation record](evidence/isolated-segment-continuation.json) preserves thirteen previously verified segments across the runtime-isolation change. Removing the single process-exit line reproduces the prior rendering script exactly; all visual inputs and encoding code remain identical. Each reused segment was rehashed before updating its continuation fingerprint.
