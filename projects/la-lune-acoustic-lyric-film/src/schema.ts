...
export type Cue={id:string;section:string;fr:SourceWord[];en:TargetWord[];startSample:number;endSample:number;visibleFrom:number;visibleUntil:number};
...
export function parseData(value:unknown):ProductionData{
 ...
 const sourceIds=new Set(fr.map(w=>w.id));if(sourceIds.size!==fr.length)throw Error('Duplicate source word');
 for(const w of en)if(!w.sourceIds.length||w.sourceIds.some(id=>!sourceIds.has(id)))throw Error('Unknown semantic source');
 for(const w of fr)if(!en.some(t=>t.sourceIds.includes(w.id)))throw Error('Unmapped French word');
 const startSample=integer(c.startSample),endSample=integer(c.endSample);if(fr.some(w=>w.startSample<startSample||w.endSample>endSample))throw Error('Cue containment');
 return {id,section:str(c.section),fr,en,startSample,endSample,visibleFrom:integer(c.visibleFrom),visibleUntil:integer(c.visibleUntil)};});
