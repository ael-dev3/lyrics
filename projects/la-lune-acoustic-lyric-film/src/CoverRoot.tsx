...
function Cover({portrait=false}:{portrait?:boolean}){
 ...
 const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}"><defs><filter id="neutral-moon" color-interpolation-filters="sRGB"><feColorMatrix type="saturate" values="0"/></filter><radialGradient id="fade"><stop offset=".8" stop-color="white"/><stop offset="1" stop-color="black"/></radialGradient><mask id="mask"><circle cx="${cx}" cy="${cy}" r="${size/2}" fill="url(#fade)"/></mask></defs><rect width="${w}" height="${h}" fill="${p.ink}"/><image filter="url(#neutral-moon)" href="${staticFile('moon-v2.png')}" x="${cx-size/2}" y="${cy-size/2}" width="${size}" height="${size}" mask="url(#mask)" opacity=".92"/>${spectrum}${content}</svg>`;
