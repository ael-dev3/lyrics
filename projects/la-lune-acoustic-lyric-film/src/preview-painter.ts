import {starOpacity,stars} from './stars.ts';
...
export function createPreviewPainter(host:HTMLElement,data:ProductionData,layouts:Layouts,bands:number[][]){
 ...
 let content:Element,title:Element,lyric:Element,moon:Element,halo:Element,shade:Element,ripple:Element,bars:Element[]=[],starNodes:Element[]=[],fr:Element[]=[],en:Element[]=[];
 ...
 return {paint(frame:number,next:Format){
  ...
  if(format!==next){host.innerHTML=sceneSvg(frame,next,data,layouts,bands);format=next;cueId=id;rebuilds++;content=required('#scene-content');title=required('#title-layer');lyric=required('#lyric-layer');moon=required('#moon-light');halo=required('#moon-atmosphere');shade=required('#moon-shade');ripple=required('#instrumental-ripple');bars=[...host.querySelectorAll('[data-band]')];starNodes=[...host.querySelectorAll('[data-star]')];words();}
  ...
  stars(next).forEach((star,i)=>{for(const node of starNodes[i]!.children)set(node,'opacity',String(starOpacity(star.hr,star.opacity,frame)));});
