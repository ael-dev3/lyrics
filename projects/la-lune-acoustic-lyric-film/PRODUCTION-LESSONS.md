...
Native canvas buffers do not necessarily trigger JavaScript heap collection promptly. Long exports require explicit periodic collection and a single full-resolution worker under constrained memory, even when a short diagnostic is fast. Verify sustained resident memory and retain per-segment hashes so interruption does not invalidate completed work. This runtime safeguard changes resource reclamation, not frame data.
