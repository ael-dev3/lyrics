...
The covers use the AI-assisted Moon illustration displayed in neutral silver with dedicated code-native poster layouts in [CoverRoot.tsx](../src/CoverRoot.tsx). Text is set with the bundled Space Grotesk font. The cover export script verifies dimensions; local review covers small sizes and a conservative centered crop. [Cover identity and review](../evidence/cover-assets.json) records the files and limits. The source-art prompt is preserved in [artwork provenance](../source/moon-artwork-prompt.md).
...
Both complete videos and all supporting files were packaged and copied to the requested Desktop folder. [Local kit manifest](../evidence/delivery-receipt.json), [Desktop file verification](../evidence/desktop-delivery-receipt.json) and [checksums](../evidence/delivery-checksums.sha256) record all fourteen files. The manifest records the exact source commit; later receipt documentation does not alter delivered bytes. The Celestial v3 folder uses a distinct name from the historical v2 delivery.
