...
export function checkProductionReview(raw:unknown,identity:PreviewIdentity,current:Record<string,string>,cueIds:string[],authorization?:unknown){
 ...
 if(review.reviewMode==='owner-approved-preview'){
  ...
  const accepted=((identity.revision==='preview-v3-audio-led'&&auth.authorizationId==='kazhdyy-v3-production-2026-09-17')||(identity.revision==='preview-v4-vocal-response'&&auth.authorizationId==='kazhdyy-v4-production-2026-09-17')||(identity.revision==='preview-v5-section-dynamics'&&auth.authorizationId==='kazhdyy-v5-production-2026-09-17'))&&auth.previewAccepted===true;
