...
import {artworkPose,smooth} from './motion.ts';
...
export function createPreviewPainter(host:HTMLElement,data:ProductionData,layouts:Layouts,bands:number[][]){
 ...
 let art:Element,content:Element,title:Element,lyricLayer:Element,grain:Element;
 ...
 return {
  paint(frame:number,nextFormat:Format){
   ...
   if(format!==nextFormat){
    ...
    grain=host.querySelector('#shadow-seed')!;art=host.querySelector('#art-pose')!;content=host.querySelector('#scene-content')!;title=host.querySelector('#title-layer')!;lyricLayer=host.querySelector('#lyric-layer')!;
   ...
   set(grain,'seed',String(shadowSeed(frame)));
   ...
   const heights=spectrumHeights(frame,m.values);
   bars.forEach((bar,i)=>{const height=heights[i]??3;set(bar,'d',`M ${(railX+i*railW/63).toFixed(3)} ${railY} v ${(-height).toFixed(3)}`);});
