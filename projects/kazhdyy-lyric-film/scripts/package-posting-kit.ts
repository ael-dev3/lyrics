...
for(const [format,platform,dimensions] of [['landscape','YouTube','1920x1080'],['portrait','TikTok','1080x1920']] as const){
 const name=`Kazhdyy-${format}-${dimensions}-60fps.mp4`,source='output/'+name,technical=read(`evidence/${name}.verification.json`),focus=read(`evidence/${format}-${name}.focus-verification.json`);
 ...
 const cover=platform==='YouTube'?'Kazhdyy-YouTube-Thumbnail-1920x1080.jpg':'Kazhdyy-TikTok-Cover-Profile-1200x1600.jpg';transfers.push({source:'publishing/'+cover,file:platform+'/'+cover});
 for(const kind of ['Title','Description'])transfers.push({source:`publishing/${platform}-${kind}.txt`,file:`${platform}/${platform}-${kind}.txt`});
}
for(const lang of ['ru','en','bilingual'])transfers.push({source:`publishing/captions/Kazhdyy-${lang}.srt`,file:`Captions/Kazhdyy-${lang}.srt`});
...
writeFileSync(join(root,'START-HERE.txt'),`каждый, кто делал тебе больно — забей, лерочка\n\nYouTube: use the landscape MP4, title, description and 1920×1080 thumbnail in YouTube/.\nTikTok: use the portrait MP4, title, description and portrait 1200×1600 profile cover in TikTok/.\n\nBoth videos contain the complete recording with Russian and English word highlighting. Duration: 3:33.300; 60 fps. Original AAC audio is preserved.\n\nCaptions/ contains optional line-level Russian, English and bilingual SRT files. The films already include burned-in bilingual words; enabling additional captions can duplicate them.\n\nThe cover JPEGs are AI-assisted promotional adaptations of the source illustration. Source and credit details are in each platform description.\nChecksums and the delivery manifest verify these files. No platform upload has been performed.\n`);
