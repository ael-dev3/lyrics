...
if(!GlobalFonts.registerFromPath('public/Oswald-Medium.ttf','LyricSans'))throw Error('Font load failed');
const ctx=createCanvas(1920,1920).getContext('2d');
const layouts:Layouts={landscape:{width:1920,height:1080,fontSize:78,lineHeight:96,safeX:96,cues:{}},portrait:{width:1080,height:1920,fontSize:82,lineHeight:104,safeX:86,cues:{}}};
for(const [format,l] of Object.entries(layouts)){
 ctx.font='500 '+l.fontSize+'px LyricSans';
 const x0=format==='landscape'?986:86,maxWidth=format==='landscape'?838:908,gap=ctx.measureText(' ').width;
 ...
 for(const cue of data.cues){const ru=fit(cue.ru,format==='landscape'?425:1220),en=fit(cue.en,format==='landscape'?745:1560);l.cues[cue.id]={ru:ru.boxes,en:en.boxes,ruRows:ru.rows,enRows:en.rows};}
}
