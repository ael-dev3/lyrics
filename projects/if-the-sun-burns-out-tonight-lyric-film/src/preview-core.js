...
export function spectrumGeometry(width, height, energy, exponent = 1.18) {
  const gap = Math.min(2, width * .0028);
  const bandWidth = Math.max(.25, (width - gap * (energy.length - 1)) / energy.length);
  return energy.map((value, index) => {
    const level = Math.max(0, Math.min(1, value));
    const barHeight = Math.min(height, 1 + Math.pow(level, exponent) * Math.max(0, height - 2));
    return {x: index * (bandWidth + gap), y: height - barHeight, width: bandWidth, height: barHeight, level};
  });
}
