...
const alternatives=[parseAlignment('analysis/whisper-vocals-lines.json'),parseAlignment('analysis/mms-mix-lines.json')];
...
const cues=text.map(raw=>{const c=object(raw);const ru=array(c.ru).map(raw=>{const w=object(raw),index=cursor++,selected=primary[index];if(!selected)throw Error('Missing candidate');
 ...
 ledger.push({id:w.id,text:w.text,selectedMethod:'MMS isolated vocals, section bounds; PROVISIONAL',selected:{startSample,endSample},candidates:[{method:'MMS-vocals-section',...selected},...others.map((v,i)=>({method:i===0?'Whisper-vocals-line':'MMS-mix-line',...v}))],candidateSpreadMs:spread,listeningReview:'incomplete',waveformSpectrogramReview:'pending',reviewReasons:[...(spread>25?['candidate spread exceeds 25 ms']:[]),...(endSample-startSample<sampleRate*.08?['brief word, inspect onset and release']:[])]});
