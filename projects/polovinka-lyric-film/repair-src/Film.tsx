...
import {FPS,FRAMES,TRACK,P,layout} from '../src/config';
import {cues,cueAt,wordActive,smooth} from '../src/timing';
...
export const Film:React.FC<{portrait?:boolean;sampleFrame?:number;forceCue?:number;forceActive?:boolean;staticBackground?:boolean}>=({portrait=false,sampleFrame,forceCue,forceActive,staticBackground=false})=>{
 ...
 const introEnd=Math.ceil(cues[0]!.startSample/800)-12;
 const titleMode=!cue&&(f<introEnd||t>141),lyricOpacity=forceCue===undefined?state?.opacity??0:1;
 ...
 return <AbsoluteFill style={{background:P.ink,color:P.cream,fontFamily:'Oswald',overflow:'hidden'}}><style>{css}</style><AbsoluteFill style={{opacity:forceCue!==undefined?1:globalFade}}>
  ...
  {titleMode&&<div style={{position:'absolute',top:portrait?1110:701,left:L.left,width:L.lyricWidth,textAlign:'center',opacity:t<14?smooth(1,2,t)*(1-smooth(introEnd-12,introEnd,f)):smooth(141,143,t)}}><div style={{fontFamily:'Cormorant',fontWeight:600,fontSize:portrait?91:88,lineHeight:1.2,color:P.teal}}>{TRACK.translation}</div><div style={{fontSize:portrait?24:23,letterSpacing:4,marginTop:25,color:P.cream}}>ТАНЦЫ МИНУС</div></div>}
