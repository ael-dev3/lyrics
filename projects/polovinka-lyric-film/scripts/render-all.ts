...
const parts = integer('parts', 8, FRAMES), concurrencyPerWorker = Math.floor(totalConcurrency / workers);
...
const serveUrl = await bundle({entryPoint: resolve('src/index.tsx'),webpackOverride:config=>({...config,cache:false})});
...
for (const id of ['PolovinkaYouTube', 'PolovinkaTikTok']) {
...
writeFileSync('evidence/render-options.json', JSON.stringify({
  fps: FPS, frames: FRAMES, compositions: ['PolovinkaYouTube', 'PolovinkaTikTok'],
...
const jobs = Array.from({length:parts},(_,index)=>['youtube','tiktok'].map(kind=>({kind,part:index+1}))).flat();
let cursor = 0;
