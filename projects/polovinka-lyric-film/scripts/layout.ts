...
const count=JSON.parse(readFileSync('src/cues.json','utf8')).length;
const flag=(name:string)=>process.argv.find(v=>v.startsWith('--'+name+'='))?.slice(name.length+3);
const serveUrl=flag('serve-url')??await bundle({entryPoint:resolve('src/index.tsx'),webpackOverride:config=>({...config,cache:false})});
...
for(const kind of ['YouTube','TikTok']){
 const composition=await selectComposition({serveUrl,id:'Layout'+kind,...browserOptions});
 const rows:{cue:string;state:number;words:number[][]}[]=[];
 await renderFrames({serveUrl,composition,...browserOptions,inputProps:{},onStart:()=>{},onFrameUpdate:()=>{},outputDir:resolve('evidence/layout-'+kind),imageFormat:'png',concurrency:3,onBrowserLog:l=>{if(l.text.startsWith('LAYOUT_OK '))rows.push(JSON.parse(l.text.slice(10)));}});
