...
const serveUrl = sharedBundle ?? await bundle({entryPoint: resolve('repair-src/index.tsx'),webpackOverride:config=>({...config,cache:false})});
const browserExecutable = flag('browser-executable');
const browserOptions = browserExecutable ? {browserExecutable} : {};
const composition = await selectComposition({serveUrl, id: portrait ? 'PolovinkaTikTok' : 'PolovinkaYouTube', ...browserOptions});
