...
A full rerun also needs the locked animation and AAC soundtrack, font files, poster, generated `public/science.json` and `public/motion.json`, source alignment observations and analysis PCM. Large licensed media, dependencies, generated frames and PCM are not duplicated here. `analysis/manifest.json` records soundtrack and PCM hashes, filter coefficients and clocks. Presentation features can be derived by the preserved analysis scripts with their documented input layout. The final movie remains the locally delivered artifact; this documentation change does not claim a newly uploaded release asset.
