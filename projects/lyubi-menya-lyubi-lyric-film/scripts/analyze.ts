...
const media=JSON.parse(readFileSync('source/media-manifest.json','utf8')) as {audio:{presentedDuration:number;sampleRate:number}};
if(media.audio.sampleRate!==SR)throw Error('Analysis sample rate does not match DSP configuration');
const bytes=readFileSync('analysis/audio-delivery.f32'),pcm=new Float32Array(bytes.buffer,bytes.byteOffset,bytes.byteLength/4),frames=Math.ceil(media.audio.presentedDuration*60),data=new Float32Array(frames*64),start=Date.now();
// AAC decoding exposes a short padded tail beyond the presented container duration.
// Keep original PCM intact while measuring only actual presentation frames.
for(let i=0;i<64;i++){const values=measure(pcm,bands[i]!);values.slice(0,frames).forEach((v,f)=>{data[f*64+i]=v;});if(i%8===7)console.log({bands:i+1,seconds:(Date.now()-start)/1000});}
