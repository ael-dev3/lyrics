...
const verse:Line[]=[
 ...
 {key:'v2',ru:'Любит не меня уже который год',en:'Has|0 loved,|0 but|1 not|1 me,|2 for|4,5 years|4,5 now|3',note:'English present perfect preserves the continuing Russian present with elapsed duration and the explicit not-me contrast. Уже supplies now. Который год is the semantic basis of for years; display for at the duration construction onset and years on the noun год. For is a grammatical completion, not a literal translation of который. Preserve the unstated alternative object and each independently sung Russian event.'},
...
for(const cue of text.filter(c=>c.key==='v2')){
 for(const word of cue.en){
  if(word.text==='for')Object.assign(word,{focusSourceIds:[cue.ru[4]!.id],focusRationale:'The entire который год construction means for years. Its English duration preposition enters on the construction onset который; no independent English timestamp is invented.'});
  if(word.text==='years')Object.assign(word,{focusSourceIds:[cue.ru[5]!.id],focusRationale:'Keep the full который год duration meaning in sourceIds, while the visible noun years follows the independently sung noun год.'});
 }
}
