...
The current production edition **`rose-paper-v1.1.0`** uses the accepted **`preview-v3-duration-focus`** input identity. Current [review and replacement-render authorization](evidence/render-authorization.json) are recorded, and the synchronization gate passes. Both replacement films pass complete media and decoded-focus verification; all 14 files in the new Desktop posting kit match the [delivery records](evidence/delivery-receipt.json).

The earlier **`rose-paper-v1.0.0`** edition used **`preview-v2-rose-paper`**. Its [archived receipt](evidence/releases/rose-paper-v1.0.0/delivery-receipt.json), review and final-file checks remain evidence for those v1 bytes. Current approval retains the prior comprehensive review for the 31 unchanged cues and records acceptance of the corrected wording, focus and releases in LM-002 and LM-013. Source preparation and technical checks remain supporting evidence, not listening telemetry.
...
Choose the smallest meaningful correspondence rather than forcing one English token per Russian word. In the current map all 147 Russian events remain independent; 125 have one English token and 22 have a justified two-word expansion. The complete English verb, article/noun or phrasal expression shares its source event when no finer performed anchor exists.
...
Keep each edition's receipt, frozen identity, captions, decoded proofs and verification records associated with its exact bytes. The earlier v1.0.0 evidence is archived under [`evidence/releases/rose-paper-v1.0.0/`](evidence/releases/rose-paper-v1.0.0/). The replacement kit uses `--edition rose-paper-v1.1.0` and a new destination, preserving the historical handoff. It was built from source checkpoint `d2394d583a89634077cd69020396279da2194eb6`; both final films passed verification and all 14 files passed the separate [Desktop copy check](evidence/desktop-delivery-receipt.json).
