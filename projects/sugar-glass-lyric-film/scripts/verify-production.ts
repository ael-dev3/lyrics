...
const probe=object(JSON.parse(command('ffprobe',['-v','error','-threads','4','-count_frames','-show_streams','-show_format','-show_frames','-show_entries','frame=best_effort_timestamp_time,media_type','-of','json',output]))),streams=array(probe.streams).map(object),video=streams.find(s=>s.codec_type==='video'),audio=streams.find(s=>s.codec_type==='audio');
