import {visualizer} from '../src/scene.ts';
import {readFileSync,writeFileSync} from 'node:fs';
import {createHash} from 'node:crypto';
import {bands,measure,SR} from './dsp.ts';
const bytes=readFileSync('analysis/audio-delivery.f32'),pcm=new Float32Array(bytes.buffer,bytes.byteOffset,bytes.byteLength/4),frames=Math.ceil(pcm.length/2/SR*60),data=new Float32Array(frames*64),start=Date.now();
