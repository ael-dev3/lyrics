...
export const Film:React.FC<{format:Format}>=({format})=>{
 ...
 const [handle]=useState(()=>delayRender('Load bilingual font'));
 ...
 return <AbsoluteFill style={{background:palette.night,overflow:'hidden'}}>
  ...
  <OffthreadVideo src={staticFile('footage.mp4')} muted style={{position:'absolute',width:l.width,height:portrait?608:1080,top:portrait?225:0,objectFit:'contain',maskImage:portrait?'linear-gradient(transparent,black 12%,black 82%,transparent)':undefined}}/>
