...
import {AbsoluteFill,Audio,OffthreadVideo,Freeze,staticFile,useCurrentFrame,delayRender,continueRender,cancelRender,getRemotionEnvironment} from 'remotion';
...
export const Film:React.FC<{format:Format;production?:boolean}>=({format,production=false})=>{
 if(getRemotionEnvironment().isRendering&&(!production||authorization.status!=='authorized'||!authorization.fullRenderAuthorized))throw Error('PRODUCTION BLOCKED: use the approved render entry point.');
 ...
 return <AbsoluteFill style={{background:palette.night,overflow:'hidden'}}>
  ...
  <Freeze frame={Math.min(frame,13876)}><OffthreadVideo src={staticFile('footage.mp4')} muted style={{position:'absolute',width:l.width,height:portrait?650:1080,top:portrait?265:0,objectFit:'contain',maskImage:portrait?'linear-gradient(transparent,black 12%,black 82%,transparent)':undefined}}/></Freeze>
