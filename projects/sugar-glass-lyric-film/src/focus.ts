...
export function activeSource(cue:Cue,frame:number,data:ProductionData){return new Set(cue.source.filter(w=>wordActive(w,frame,data)).map(w=>w.id));}
export function activeTargets(cue:Cue,frame:number,data:ProductionData){const source=activeSource(cue,frame,data);return new Set(cue.target.filter(w=>w.sourceIds.some(id=>source.has(id))).map(w=>w.id));}
...
export function visibleCues(data:ProductionData,frame:number){const sample=Math.round(frame/data.fps*data.sampleRate);return data.cues.filter(c=>sample>=c.visibleFrom&&sample<c.visibleUntil);}
