...
export type Cue={id:string;section:string;layer:'main'|'backing';source:SourceWord[];startSample:number;endSample:number;visibleFrom:number;visibleUntil:number};
...
export function parseData(value:unknown):ProductionData{
 ...
 return {id,section:str(c.section),layer,source,startSample,endSample,visibleFrom:integer(c.visibleFrom),visibleUntil:integer(c.visibleUntil)};});
