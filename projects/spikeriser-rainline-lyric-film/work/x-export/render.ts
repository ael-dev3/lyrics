...
const acceptance=read('work/x-export/owner-acceptance.json');
const identity=read('evidence/preview-identity.json');
if (acceptance.song!==SONG_ID || acceptance.revision!==REVISION ||
    acceptance.authorizationId!=='rainline-v4-desktop-export-2026-09-24' ||
...
for (const path of ['work/x-export/Scene.tsx','work/x-export/entry.tsx','work/x-export/render.ts']) if (hash(readFileSync(path))!==acceptance.rendererHashes[path]) throw new Error(`Delivery renderer changed: ${path}`);
...
const serveUrl=await bundle({entryPoint:resolve('work/x-export/entry.tsx'),publicDir:resolve('public')});
const inputProps={deliveryPermit:true};
const composition=await selectComposition({serveUrl,id:'Rainline-X',inputProps,...browser});
if(composition.width!==1080||composition.height!==1900||composition.fps!==30||composition.durationInFrames!==7418)throw new Error('Unexpected X composition');
if(proof){
 for(const time of [12,101,177,184]) await renderStill({serveUrl,composition,inputProps,frame:Math.round(time*30),output:resolve(`work/x-export/proof-${time}.png`),...browser});
 console.log('X frame proofs ready');
}else{
 const folder=resolve('../../../../outputs');
 mkdirSync(folder,{recursive:true});
 const picture=resolve('work/x-export/Rainline-X-picture.mp4'),output=resolve(folder,'Rainline-X.mp4');
 if(existsSync(picture)||existsSync(output))throw new Error('Refusing to overwrite existing export');
 let last=0;
 await renderMedia({serveUrl,composition,inputProps,...browser,outputLocation:picture,codec:'h264',pixelFormat:'yuv420p',imageFormat:'png',colorSpace:'bt709',crf:16,x264Preset:'slow',encodingMaxRate:'20M',encodingBufferSize:'40M',gopSize:60,muted:true,concurrency:4,onProgress:p=>{if(Date.now()-last>10000){last=Date.now();console.log(JSON.stringify({progress:p.progress,renderedFrames:p.renderedFrames,encodedFrames:p.encodedFrames}));}}});
 ...
 writeFileSync('work/x-export/render-result.json',JSON.stringify({output,previewIdentitySha256:identity.identitySha256,authorizationId:acceptance.authorizationId,rendererHashes:acceptance.rendererHashes,finishedAt:new Date().toISOString()},null,2)+'\n');
