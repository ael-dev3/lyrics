...
// The accepted 1080x1920 scene is sampled on its original 60 Hz clock.
// Only the outer ten pixels at the top and bottom are cropped for X web.
export const XScene: React.FC<{deliveryPermit: boolean}> = ({deliveryPermit}) => {
  ...
  useLayoutEffect(() => {
    ...
    paintCity(ctx, frame * 2, 'portrait');
    if (!released.current) {released.current=true;continueRender(handle);}
  }, [ready, frame, handle]);
  return <AbsoluteFill style={{background:'#081019',overflow:'hidden'}}><canvas ref={canvas} width={1080} height={1920} style={{position:'absolute',left:0,top:-10,width:1080,height:1920,imageRendering:'pixelated'}}/></AbsoluteFill>;
};
