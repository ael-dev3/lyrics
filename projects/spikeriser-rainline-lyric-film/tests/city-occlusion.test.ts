...
function traceContext(width: number, height: number, captureFocus = false) {
  ...
  const context = {
    ...
    fillRect(this: {fillStyle: string; globalAlpha: number}, x: number, y: number) {
  ...
  return {context, calls, focusPixels};
}
