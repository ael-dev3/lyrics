...
| Canvas motion | Separate source-time layers for dense near/far rain, ring light, lightning, one recurring rail consist, taxis, five distant hover cars, sidewalk umbrella walkers with a gait cycle, cat, steam, and lyric/spectrum surfaces. Keep their depth order legible. |
...
Use the project's 5 × 7 pixel lettering. Keep words stably placed **inside the original dark glass apertures**. The v4 compositor draws glyphs with restrained emissive color and clips them to those apertures; it does not cover shopfronts, train metal, or sign bevels with new solid panels. The active word brightens in place; inactive words stay readable. There is no floating karaoke strip. The current 18-cue route uses **five shop-row phrases, five rail-car LED phrases, seven building-ad phrases, and one airship LED word** (`CH-03`, “Lightning”). Shop phrases flow across adjacent fascia signs; portrait also repeats the whole line on the municipal board for phone-size reading, within an aperture that ends before the cat window. Other airship flights carry ambient service text. Train words travel on one coherent, recurring consist on the existing viaduct. Letter size and row count adapt to each native aperture; the long final train phrase takes the two LED rows “WATCHING THE” / “LAST AXIS TRAIN” without enlarging the car. Portrait pushes toward the rail for train cues to make that native lettering easier to read.
