...
const expected=(read('source/text-and-mapping.json') as {ru:{text:string}[]}[]).flatMap(c=>c.ru.map(w=>w.text));
assert.deepEqual(d.cues.flatMap(c=>c.ru.map(w=>w.text)),expected,'Complete independently performed sequence');
assert.equal(d.frames,Math.ceil(d.duration*d.fps));assert.equal(bands.length,d.frames);
...
for(let ci=0;ci<d.cues.length;ci++){
 ...
 for(const format of ['landscape','portrait'] as const){
  const l=layouts[format];const geo:CueLayout|undefined=l.cues[c.id];assert.ok(geo);assert.deepEqual(geo.ru.map(w=>w.id),c.ru.map(w=>w.id));assert.deepEqual(geo.en.map(w=>w.id),c.en.map(w=>w.id));
  for(const box of [...geo.ru,...geo.en])assert.ok(box.x>=l.safeX&&box.x+box.width<=l.width-l.safeX&&box.y<l.height-120);
  for(const f of frames){
   const palette=paletteAt(f/d.fps),svg=sceneSvg(f,format,d,layouts,bands),visible=Math.round(f/d.fps*d.sampleRate)>=c.visibleFrom&&Math.round(f/d.fps*d.sampleRate)<c.visibleUntil;
   ...
   for(const word of [...c.ru,...c.en]){
    ...
    const source:SourceWord[]='sourceIds'in word?c.ru.filter((w:SourceWord)=>targetFocusIds(word as TargetWord).includes(w.id)):c.ru.filter(w=>sourceFocusIds(c,word.id).includes(w.id));
 ...
 semantic.push({id:c.id,ru:c.ru,sourceDisplayGroups:c.ru.map(w=>({id:w.id,sourceIds:sourceFocusIds(c,w.id)})),en:c.en.map(w=>({...w,intervals:c.ru.filter(s=>w.sourceIds.includes(s.id)).map(s=>({startSample:s.startSample,endSample:s.endSample})),focusIntervals:c.ru.filter(s=>targetFocusIds(w).includes(s.id)).map(s=>({startSample:s.startSample,endSample:s.endSample})),rule:'lexical correspondence retained; paired display group in both languages uses source interval union with exclusive ends, no bridge across gaps or unrelated source events'}))});
}
for(let f=0;f<d.frames;f++)for(const v of spectrum(f,bands))assert.ok(Number.isFinite(v)&&v>=0&&v<=1);
assert.ok(maxQuantizationMs<=1000/d.fps/2+.0001);
const result={status:'PASS',cues:d.cues.length,sourceWords:d.cues.reduce((n,c)=>n+c.ru.length,0),targetWords:d.cues.reduce((n,c)=>n+c.en.length,0),boundaryStatesBothFormats:states,visibleWordColors:wordColors,maxFrameQuantizationMs:maxQuantizationMs,frames:d.frames,scope:'Source sequence, meaning-map coverage, geometry data, SVG colors and measured display bounds. This does not complete actual-audio review.'};
