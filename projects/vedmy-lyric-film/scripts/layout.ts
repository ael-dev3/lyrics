...
if(!GlobalFonts.registerFromPath('public/fonts/CormorantGaramond-Semibold.ttf','EmberSerif'))throw Error('Font load failed');
const ctx=createCanvas(1920,1920).getContext('2d');
const layouts:Layouts={landscape:{width:1920,height:1080,fontSize:62,lineHeight:82,safeX:96,cues:{}},portrait:{width:1080,height:1920,fontSize:74,lineHeight:100,safeX:86,cues:{}}};
for(const [format,l] of Object.entries(layouts)){
 ctx.font='600 '+l.fontSize+'px EmberSerif';
 const x0=format==='landscape'?1120:96,maxWidth=format==='landscape'?700:888,gap=ctx.measureText(' ').width;
 ...
 for(const cue of data.cues){const ru=fit(cue.ru,format==='landscape'?500:1210),en=fit(cue.en,format==='landscape'?733:1480);l.cues[cue.id]={ru:ru.boxes,en:en.boxes,ruRows:ru.rows,enRows:en.rows};}
}
