...
GlobalFonts.registerFromPath('public/fonts/CormorantGaramond-Semibold.ttf','EmberSerif');
...
for(const cue of data.cues){const layout=l.cues[cue.id]!;for(const w of [...layout.ru,...layout.en]){
 const size=l.fontSize,canvas=createCanvas(Math.ceil(w.width+8),Math.ceil(size*1.5)),ctx=canvas.getContext('2d');ctx.font=`600 ${size}px EmberSerif`;ctx.fillStyle='white';ctx.fillText(w.text,w.x-Math.floor(w.x)+3,size);const rgba=ctx.getImageData(0,0,canvas.width,canvas.height).data;
