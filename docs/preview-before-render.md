...
- The main review preview must include the full intended visual composition: source footage or artwork, animation, transitions, lyrics and visualizer. Do not silently omit a layer or hand off a lyrics-only diagnostic as the complete preview. Label isolated diagnostics separately and keep the complete preview available.
...
- Verify the actual browser picture in every delivery format after a fresh load, seeking, playback and aspect-ratio switching. Confirm that source motion visibly advances beneath the lyrics; a healthy media clock, decoded dimensions, geometry audit or offline composite does not prove the browser displays the video. Check a known nonblack source moment and the ending. Provide an always-available recovery control that restores the media surface while retaining position and review notes, including when the browser reports no error. A missing or frozen intended visual layer blocks preview handoff until restored or explicitly identified as unresolved.
