...
const width=160,height=90,size=width*height;
...
const candidates=changes.filter((c,i)=>c.mad>25&&changes.slice(Math.max(0,i-2),i+3).every(n=>n.mad<=c.mad)).map(c=>({frame:c.frame,time:c.pts,mad:Number(c.mad.toFixed(4))}));
writeFileSync(join(out,'cut-candidates.json'),JSON.stringify(candidates,null,2)+'\n');
